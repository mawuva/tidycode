...
def create_mock_runner(name: str, category: str = "quality") -> mock.MagicMock:
    ...
    mock_runner.get_command_spec.return_value = CommandSpec(
        command=[name, "--check", "."], display_name=name, cwd=Path("."), is_tool=True
    )
...
def test_run_plugins_with_quality_tools():
    ...
    with (
        ...
        mock.patch("tidycode.runner.subprocess.load_plugins_from") as mock_load,
        mock.patch("tidycode.plugins.registry.registry.filter") as mock_filter,
        mock.patch(
...
def test_run_plugins_dry_run():
    ...
    with (
        ...
        mock.patch("tidycode.runner.subprocess.load_plugins_from") as mock_load,
        mock.patch("tidycode.plugins.registry.registry.filter") as mock_filter,
        mock.patch("tidycode.plugins.runner.base_runner.BaseRunner.run"),
    ):
        # Mock configuration
        mock_config.return_value = {
            "target": ".",
        ...
        # Run plugins in dry run mode
        run_plugins(
            category="quality", tools=["black"], dry_run=True, live=False, verbose=False
        )

        # Verify dry run was called
        mock_plugin.run.assert_called_once_with(
            target=Path("."), check_only=False, dry_run=True, live=False, verbose=False
        )
...
def test_run_plugins_tool_not_found():
    ...
    with (
        ...
        mock.patch("tidycode.runner.subprocess.load_plugins_from"),
        mock.patch("tidycode.plugins.registry.registry.filter") as mock_filter,
        mock.patch("tidycode.runner.subprocess.print_warning") as mock_warning,
    ):
        # Mock configuration
        mock_config.return_value = {
            "target": ".",
...
def test_run_plugins_with_custom_path():
    ...
    with (
        ...
        mock.patch("tidycode.runner.subprocess.load_plugins_from"),
        mock.patch("tidycode.plugins.registry.registry.filter") as mock_filter,
        mock.patch("tidycode.runner.subprocess.run_multiple_commands"),
    ):
        # Mock configuration
        mock_config.return_value = {
            "target": ".",
        ...
        mock_plugin.get_command_spec.assert_called_once_with(
            target=Path("/custom/path"), check_only=False
        )
...
def test_run_plugins_no_tools_specified():
    ...
    with (
        ...
        # Run plugins without specifying tools
        run_plugins(category="quality", dry_run=False, live=False, verbose=False)
